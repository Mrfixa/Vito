...
class ZoneService extends BaseService implements ZoneServiceInterface
{
    ...
    protected function createPoint($coordinates): string
    {
        $polygon = [];
        $lastcord = null;
        foreach (explode('),(', trim($coordinates, '()')) as $index => $single_array) {
            ...
            $polygon[] = [(float)$coords[1], (float)$coords[0]]; // [lng, lat] GeoJSON order
        }
        if ($lastcord !== null) {
            $polygon[] = [(float)$lastcord[1], (float)$lastcord[0]];
        }
        return json_encode(['type' => 'Polygon', 'coordinates' => [$polygon]]);
    }
    ...
    public function getZones(array $criteria = []): array
    {
        ...
        foreach ($allZones as $item) {
            $zoneCoordinate = json_decode($item->coordinates, true);
            $allZoneData[] = formatCoordinates($zoneCoordinate['coordinates'][0] ?? []);
        }
        return $allZoneData;
    }
